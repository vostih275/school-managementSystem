...
async function fetchFreshProfile(token) {
  ...
  try {
    ...
    const response = await fetch(`${window.API_CONFIG?.API_BASE_URL || "http://localhost:5000/api"}${profileEndpoint}`, {
      method: "GET",
      headers: {
  ...
  if (photoElement) {
    ...
    if (photoUrl) {
      ...
      if (photoUrl && photoUrl.includes('localhost')) {
        photoUrl = photoUrl.replace(/^http:\/\/localhost(:\d+)?/, window.API_CONFIG?.BASE_URL || "http://localhost:5000");
      }
...
async function updateProfile() {
  ...
  try {
    const response = await fetch((window.API_CONFIG?.BASE_URL || 'http://localhost:5000') + "/api/profile", {
      method: "PUT",
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
...
async function handlePhotoUpload(event) {
  ...
  try {
    const response = await fetch((window.API_CONFIG?.BASE_URL || 'http://localhost:5000') + '/api/students/profile/photo', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
      },
      ...
      if (photoElement) {
        ...
        if ((!photoUrl || photoUrl === '') && data.profile?.photo) {
          ...
          if (data.profile.photo.startsWith('/')) {
            photoUrl = `${window.API_CONFIG?.BASE_URL || "http://localhost:5000"}${data.profile.photo}`;
          } else if (!data.profile.photo.startsWith('http')) {
            photoUrl = `${window.API_CONFIG?.BASE_URL || "http://localhost:5000"}/uploads/${data.profile.photo}`;
          } else {
            photoUrl = data.profile.photo;
          }
          console.log('Using profile.photo URL:', photoUrl);
...
async function changePassword() {
  ...
  try {
    const response = await fetch((window.API_CONFIG?.BASE_URL || 'http://localhost:5000') + "/api/profile/change-password", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
